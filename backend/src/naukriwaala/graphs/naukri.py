"""This module contains the Naukri graph."""
...
from naukriwaala.graphs.login import login_student
from naukriwaala.graphs.registration import register_student
...
from naukriwaala.schemas import ValidatorCall
from naukriwaala.utils.chat import (
...
@dataclass
class NaukriState:
    """The state tracks the progress of the Naukri graph."""
    ...
    naukri_queries: list[NaukriQueryUnion] = field(default_factory=list)

    def __post_init__(self) -> None:
        """Post-initialization processes."""

        # 1.
        self.last_graph_run_results_cache_key = f"{REDIS_CACHE_PREFIX_GRAPH_NAUKRI}_last_graph_run_results_{self.session_id}"


@dataclass
class NaukriDeps:
    """This class contains dependencies used by nodes in the Naukri graph."""
...
@dataclass
class SelectStudentOrAssistant(BaseNode[NaukriState, NaukriDeps, NaukriFlowResults]):
    ...
    def _update_system_message(
        self, *, ctx: GraphRunContext[NaukriState, NaukriDeps]
    ) -> None:
        """Update the system message for the Naukri agent with the correct set of
        ...
        NB: The assumption here is that the very first message in the Naukri agent chat
        ...
        ctx.deps.chat_history[0]["content"] = NaukriPrompts.system_messages[
            "naukri_agent"
    ...
    async def call_next_assistant(
        ...
        *,
        assistant_name: str,
        ctx: GraphRunContext[NaukriState, NaukriDeps],
        explanation_for_assistant_call: str,
    ) -> None:
        """Call the next assistant in the graph and get its graph run results. For each
        assistant call, the results from its graph run are summarized by the Naukri
        ...
        match assistant_name:
            case "registration.register_student":
                graph_run_results = await register_student(
                    ...
                    naukri_query=ctx.deps.naukri_query,
        ...
        content = await get_acompletion(
            ...
            system_msg=NaukriPrompts.system_messages["summarize_assistant_response"],
            text_generation_params=ctx.deps.chat_params["text_generation_params"],
            user_msg=NaukriPrompts.prompts["summarize_assistant_response"].format(
                assistant_call_results=ctx.state.last_graph_run_results,
                explanation_for_assistant_call=explanation_for_assistant_call,
                student_message=ctx.deps.naukri_query.user_query,
            ),
        )
    ...
    async def determine_next_step(
        ...
        """Determine the next step in the Naukri process. This would either be an
        assistant call or a request for student input.

        NB: In a given conversation turn between the **student and the Naukri agent**,
        ...
        now, the Naukri agent needs to make the decision on the next step based on not
        ...
        if self.summary_of_last_assistant_call is None:
            message = NaukriPrompts.prompts["naukri_agent"].format(
        ...
        log_chat_history(
            chat_history=ctx.deps.chat_history,
            context="Naukri Agent: after determining next step",
            session_id=ctx.state.session_id,
        )
    ...
    async def generate_self_summary(
        *, ctx: GraphRunContext[NaukriState, NaukriDeps]
        ...
        """Generate a summary of the Naukri process so far, **for the Naukri agent.**
        This is an internal summarization meant to help keep track of the conversation
        for the Naukri agent and is not presented to the student.
        ...
        content = await get_acompletion(
            model=LITELLM_MODEL_CHAT,
            system_msg=NaukriPrompts.system_messages["summarize_naukri_process"],
            text_generation_params=ctx.deps.chat_params["text_generation_params"],
            user_msg=NaukriPrompts.prompts["summarize_naukri_process"].format(
        ...
        log_chat_history(
            chat_history=ctx.deps.chat_history,
            context="Naukri Agent: after process summary",
            session_id=ctx.state.session_id,
        )

    async def run(self, ctx: GraphRunContext[NaukriState, NaukriDeps]) -> Annotated[
        End[NaukriFlowResults],
        Edge(label="Select either an assistant call or ask for student input"),
    ]:
        """The run proceeds as follows:

        1. Update the Naukri agent system prompt with the correct set of assistant
            ...
            the Naukri agent graph.
        2. Ask the Naukri agent to determine the next step in the Naukri process. This
        ...
        3. If student input is required, then we stop the Naukri process.
        4. Otherwise, we call the next assistant in the graph and get its graph run
            ...
            4a. For each assistant call, the results from its graph run are summarized
                by the Naukri agent and maintained in its chat history.
            4b. Update the Naukri agent system prompt with the correct set of assistant
                names and descriptions for the next step in the Naukri process. Note
        ...
        6. A summary of the Naukri process so far is generated by the Naukri agent,
            **for** the Naurki agent.
        ...
        return End(  # type: ignore
            NaukriFlowResults(  # type: ignore
                ...
                user_id=ctx.deps.naukri_query.user_id,
            )
        )


@dataclass
class DetermineStudentIntent(BaseNode[NaukriState, NaukriDeps, dict[str, Any]]):
    """This node determines the student's intent at each step in the Naukri process.
    ...
    async def run(self, ctx: GraphRunContext[NaukriState, NaukriDeps]) -> Annotated[
        ...
        Edge(
            label="Determine the student's intent at the current step in the Naukri "
            "process"
        ),
    ]:
        """The run proceeds as follows:
        ...
        chat_history.append(
            {
                "content": ctx.deps.naukri_query.user_query,
        ...
        system_msg = NaukriPrompts.system_messages["determine_student_intent"]
        user_msg = NaukriPrompts.prompts["determine_student_intent"].format(
            conversation_history=chat_history_str
        )
        content = await get_acompletion(
            model=LITELLM_MODEL_CHAT,
...
@telemetry_timer(metric_fn=naukri_agent_hist, unit="s")
async def naukri(
    *,
    csm: AsyncChatSessionManager,
    generate_graph_diagrams: bool = False,
    ...
    naukri_query.user_id = f"Naukri_Agent_Graph_{naukri_query.user_id}"
...
async def load_state(
    *,
    ...
    NB: The Naukri agent graph is responsible for passing updated **dependencies** from
    the frontend to other graphs. However, each child graph is responsible for loading
    its own **state**. This is because the Naukri agent graph calls the other graphs

...
class BasicRecommendationEngine(BaseRecommendationEngine):
    ...
    def _calculate_location_match(self, student: StudentProfile, opportunity: Opportunity) -> float:
        ...
        return len(matching_locations) / len(student.preferred_locations) if student.preferred_locations else 0.0
    
    def _calculate_stipend_match(self, student: StudentProfile, opportunity: Opportunity) -> float:
    ...
    def _calculate_education_match(self, student: StudentProfile, opportunity: Opportunity) -> float:
        """Calculate education level match score."""
        if not student.education_level:
            return 1.0
            
        if not opportunity.course_data or 'minimum_qualification' not in opportunity.course_data:
            return 0.0
    ...
    def _calculate_sector_match(self, student: StudentProfile, opportunity: Opportunity) -> float:
    ...
    def calculate_score(self, student: StudentProfile, opportunity: Opportunity) -> Tuple[float, Optional[Dict[str, float]]]:

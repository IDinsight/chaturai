...
prod-run:
	docker-compose -f docker-compose.yml -f docker-compose.testing.yml -p naukriwaala up --build -d
...
ecr-build-push:
	docker buildx build --platform linux/amd64 -f backend/Dockerfile.scraper -t naps-scraper ./backend --load
	docker tag naps-scraper ${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/naps-scraper:latest
